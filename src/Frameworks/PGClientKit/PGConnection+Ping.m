...
-(NSError* )_pingWithParameters:(NSDictionary* )parameters {
...
-(void)_pingInBackground:(NSArray* )parameters {
	...
	@autoreleasepool {
		...
		NSParameterAssert(callback);
		NSError* error = [self _pingWithParameters:dictionary];
		if(error==nil) {
...
-(BOOL)pingWithURL:(NSURL* )url error:(NSError** )error {
	...
	NSDictionary* parameters = [self _connectionParametersForURL:url];
	if(parameters==nil) {
		[self raiseError:error code:PGClientErrorParameters];
		return NO;
	}
	
	// perform ping, set error return
	NSError* error2 = [self _pingWithParameters:parameters];
	if(error2 && error) {
		(*error) = error2;
	}
	// return success condition
	return error2 ? NO : YES;
}
